...
class RatePassengerActivity : AppCompatActivity() {
    ...
    private fun submitRating() {
        ...
        passengerId?.let { passengerId ->
            val database = FirebaseDatabase.getInstance()

            val ratingRef = database.getReference("passengers").child(passengerId).child("ratings").push()
            ratingRef.setValue(selectedRating).addOnCompleteListener { ratingTask ->
                if (ratingTask.isSuccessful) {
                    ...
                    passengerRef.get().addOnCompleteListener { passengerTask ->
                        if (passengerTask.isSuccessful) {
                            val snapshot = passengerTask.result
                            ...
                            passengerRef.child("averageRating").setValue(newAverage)
                            passengerRef.child("ratingCount").setValue(newCount).addOnCompleteListener {
